...
Si5351::Si5351(I2C_HandleTypeDef *hi2c, uint8_t i2c_addr):
    i2c_bus_addr(i2c_addr),i2c_port(hi2c)
{
...
uint8_t Si5351::si5351_write_bulk(uint8_t addr, uint8_t bytes, uint8_t *data)
{
	...
	while (HAL_I2C_IsDeviceReady(i2c_port, (uint16_t)(SI5351_BUS_BASE_ADDR<<1), 3, 100) != HAL_OK) { }

	status = HAL_I2C_Mem_Write(i2c_port,
							  (uint8_t)(SI5351_BUS_BASE_ADDR<<1),
							  addr,
...
uint8_t Si5351::si5351_read(uint8_t addr)
{
	...
	HAL_StatusTypeDef status = HAL_I2C_Mem_Read(i2c_port,
							  (uint8_t)(SI5351_BUS_BASE_ADDR<<1),
							  addr,
							  I2C_MEMADD_SIZE_8BIT,
							  (uint8_t*)(&data),
